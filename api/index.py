...
if db_url:
    ...
    app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:////tmp/codehub.db'
...
# Instead of running on every request, we run it once during the cold start.
# We wrap it in a try-except to prevent the entire app from crashing if the DB isn't ready.
try:
...
def get_next_student_id():
    ...
    if not last_student: return "CH001"
    try:
        last_id_num = int(last_student.username[2:])
        return f"CH{last_id_num + 1:03d}"
    except: return "CH001"
